...
const Leaderboard: FC = () => {
  ...
  useEffect(() => {
    const fetchEntries = async () => {
      ...
      setEntries(res.data.leaderboard.sort((a, b) => b.score - a.score));
    };
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {entries.map(person => (
              <tr key={person.username}>
                <td className="px-6 py-4 whitespace-nowrap" onClick={() => handleRowClick(person.username)}>
                  <div className="flex items-center">
                    <div className="flex-shrink-0 h-10 w-10">
                      <img className="h-10 w-10 rounded-full" src={person.profileImage} alt="" />
                    </div>
                    <div className="ml-4">
